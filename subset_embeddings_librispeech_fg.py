...
class FeatureReader(Dataset):
    def __init__(self, path='/home/***/data1t_ssd/LibriSpeech/trainSpker'):
        self.path = path
...
def main(args):
    ...
    if args.mode == 'train':
        ...
        for epoch in range(epochs):
            loss = trainphase(f_net, g_net, optimizer, 32, args.train_dir)
            print('epoch {}, training loss is {}'.format(epoch, loss))

            with torch.no_grad():
                accu = validationphase(f_net, g_net, 32, args.val_dir)
        ...
        with torch.no_grad():
            accu = testphase(f_net, g_net, 1, args.test_dir)
